...
CONTAINS
  ...
  SUBROUTINE test_init_pmove(xp, np, lz, ierr)
    ...
    write(444+me,*)'after test pmove 0',mpi_wtime()!,acc_is_present(xp),acc_is_present(iz_arr)
    call flush(444+me) 
    !
    !----------------------------------------------------------------------
    ...
    !do ip=1,np
    !   xt=modulo(xp(ip),lz)
    !   iz_arr(ip)=int(xt/dzz) 
    !enddo 
    iz_arr(1:np)= INT(MODULO(xp(1:np), lz)/dzz)    !!! Assume periodicity
    !$acc end kernels
    !$acc wait
    !$acc update host(iz_arr(1:np))
    write(444+me,*)'after test pmove 1.1'!,acc_is_present(s_counts),acc_is_present(iz_arr)
    call flush(444+me)
    !$acc kernels present(s_counts,iz_arr) 
    s_counts = 0
    DO ip = 1,np
       iz =iz_arr(ip)
       IF( iz .ne. GCLR )THEN
    ...
    !$acc end kernels
    !$acc wait
    write(444+me,*)'after test pmove 1.2'
    call flush(444+me)
    !$acc kernels present(s_displ,s_counts)
    s_displ(0) = 0
    DO i=1,nvp-1
    ...
    !$acc wait
    write(444+me,*)'after test pmove 1.3',mpi_wtime()
    call flush(444+me)
...
!write(*,*)'init_pmove  xp is_present ', is_present,'me=',me
    ...
    END IF
    !$acc wait
    write(444+me,*)'after test pmove 1.4',mpi_wtime()
    call flush(444+me)
    !----------------------------------------------------------------------
    ...
    write(444+me,*)'after test pmove 1.5',mpi_wtime()
    !write(444+me,*)'write iz_arr',iz_arr
    !write(444+me,*)'write isb',isb
    !write(444+me,*)'write ipsend',ipsend
    !write(444+me,*)'write iphole',iphole
    call flush(444+me)
    ...
    !!$acc end parallel
    !
    write(444+me,*)'after test pmove 2.0',mpi_wtime()
    call flush(444+me)
    !!!update device(htc)
    ...
    !efd $acc serial present(xp,ipfill,iphole,iz_arr)
    !$acc kernels present(xp,ipfill,iphole,iz_arr)
    DO ih_ipf=nsize,1,-1
    ...
    write(444+me,*)'after test pmove 2.1',mpi_wtime()
    call flush(444+me)
    ...
    !    call ppsum(ierr)
    write(444+me,*)'after test pmove 3',mpi_wtime()
    call flush(444+me)
    !----------------------------------------------------------------------
    !              4.  Initialize tag
    !
